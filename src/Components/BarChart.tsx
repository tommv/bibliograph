...
const BarChart: FC<{ agg: Aggregation, field: FieldDefinition, filters:FiltersType, setFilters:(filters:FiltersType)=>void }> = ({ agg, field, filters, setFilters }) => {
  const maxP10 =
    ...
    <div className="BarChart">
      <div className="bars">
        {agg.values.map(({ lowerBound, count }) => (
          ...
          >
            <span>
        <strong>{count} {field.label}<br/>in more than ${lowerBound - 1} records</strong>
            </span>
          </div>
        ))}
      </div>
